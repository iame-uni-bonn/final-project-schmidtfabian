...
def test_create_trainer_for_text_classification_model_wrong_dtype_saving_location(input):
...
@pytest.mark.parametrize("input", wrong_dtypes)
def test_create_trainer_for_text_classification_model_wrong_dtype_dataset_dict_headlines(input):
    with pytest.raises(TypeError):
        create_trainer_for_text_classification_model(
