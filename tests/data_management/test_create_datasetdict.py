...
test_dictionary_not_enough_elements =  {"date": ["25.02.2024", "26.02.2024", "27.02.2024"],
                                  "values":["Hello", "World", "!"]}
...
test_dictionary_correct_dataframe =  {"text":["Hello", "World", "!", "EPPE", "is", "fun"]}
...
def test_create_datasetdict_correct_length_datasets():
    ...
    assert len(test_datasetdict["train"]["text"])==4 and len(test_datasetdict["test"]["text"])==1 and \
    len(test_datasetdict["validation"]["text"])==1, \
...
def test_create_datasetdict_correct_columns():
    ...
    assert len(feature_names_train) == 1 and 'text' in feature_names_train and \
        feature_names_train==feature_names_test and feature_names_train==feature_names_validation, \
...
def test_create_datasetdict_no_same_value():
    ...
    assert len(train_set.intersection(test_set)) == 0 and len(train_set.intersection(validation_set)) == 0 and \
        len(test_set.intersection(validation_set)) == 0, \
