...
def test_write_mse_to_file_wrong_dtypes_filepath(input):
    with pytest.raises(TypeError):
        write_mse_to_file(value=1, filepath=input)
...
def test_write_mse_to_file_wrong_dtypes_filepath(input):
    with pytest.raises(TypeError):
        write_mse_to_file(value=input, filepath= BLD / "tests" / "mse_test.txt")

def test_write_mse_to_file_correct_file_created():
    ...
    write_mse_to_file(value=1, filepath= test_filepath)
