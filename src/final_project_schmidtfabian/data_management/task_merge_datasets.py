...
def task_merge_datasets(
    ...
    dataframe_economic_indicator = pd.read_feather(
        depends_on["dataset_economic_indicator"],
    )
    merged_dataframe = dataframe_headlines.merge(
        dataframe_economic_indicator,
        left_index=True,
        right_index=True,
        how="inner",
    )
