...
def task_create_datasetdict_for_training(
    ...
    """Creates a DatasetDict for training of text-classification model."""
    handlabeled_dataset_headlines = pd.read_excel(
        depends_on["data handlabeled"],
        sheet_name=0,
        usecols="A,B",
        skiprows=1392,
        header=None,
    ...
    handlabeled_dataset_headlines["label"] = handlabeled_dataset_headlines["label"].map(
        label_mapping,
    )
