...
create_datasetdict_deps = {
    "data handlabeled": SRC / "final_project_schmidtfabian" / "data" / "headlinesdatasetfinetuninghandlabeled.xlsx",
    "scripts" : Path("create_datasetdict.py")
}

def task_create_datasetdict_for_training(
        depends_on = create_datasetdict_deps,
        produces = BLD / "data" / "datasetdict_headlines"
        ):
    ...
    datasetdict_headlines.save_to_disk(produces)