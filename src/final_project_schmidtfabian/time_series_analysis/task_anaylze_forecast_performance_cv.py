...
def task_analyze_forecast_performance_cv(
    ...
    """Creates mean squared errors for ten different time-series splits."""
    merged_dataset = pd.read_feather(depends_on)
    merged_dataset_float64 = pd.DataFrame()
    merged_dataset_float64["cycle_values"] = merged_dataset["cycle_values"].astype(
        np.float64,
    )
    ...
    for train_idx, val_idx in tss.split(merged_dataset_float64["cycle_values"]):
        ...
        ARIMA_model = ARIMA(order=(2, 0, 1), seasonal_order=(0, 0, 0, 0))
        ARIMA_model_fitted = ARIMA_model.fit(train_ARIMA_model_data)
        predictions_ARIMA_model = ARIMA_model_fitted.predict(n_periods=len(test_data))
        ...
        mse_VAR_model = mean_squared_error(
            test_data,
            VAR_forecast_df["forecast_cycle_values"],
        )
