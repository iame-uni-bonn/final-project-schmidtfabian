...
def create_summary_statistics_dataframe(ARIMA_model):
    ...
    return pd.DataFrame(
        ARIMA_model_summary_statistics.tables[1],
    )
...
def _fail_if_wrong_dtype(ARIMA_model):
    ...
    if not isinstance(ARIMA_model, pm.ARIMA):
        ...
        msg = (
            "'ARIMA_model' has to be an 'pm.ARIMA' object, but is currently"
            f" '{current_datatype}'."
        )
        raise TypeError(
            msg,
...
def _fail_if_not_series(series):
    ...
    if not isinstance(series, pd.Series):
        msg = f"Input is not a series, it is {type(series)}."
        raise TypeError(msg)
