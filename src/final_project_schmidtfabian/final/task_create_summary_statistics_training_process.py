...
for model in ["finetuned", "zero_shot_classification"]:
    ...
    create_summary_statistics_products = {
        "classification report" : BLD / "tables" / f"classification_report_{model}_model.tex",
    ...
    def task_create_summmary_statistics_training_process(
        ...
        dataframe_labels_test_data = pd.read_feather(depends_on["test data headlines labeled"])
