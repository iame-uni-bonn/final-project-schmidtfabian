...
def webscrape_headlines_welt(starting_date, number_of_days):
    ...
    data = {"headlines date": headline_date, "headlines in a list": list_of_headlines}
    headline_dataframe = pd.DataFrame(data)
    headline_dataframe["headlines date"] = pd.to_datetime(
        headline_dataframe["headlines date"],
        dayfirst=True,
    )
    return headline_dataframe.set_index("headlines date")
...
def _collect_headlines_Welt(date, session):
    ...
    categories, headlines = zip(
        *[
            item.split(" - ", 1) if " - " in item else ["", item]
            for item in category_subheadline_headline
        ],
    )
    return [
        headline
        for category, headline in zip(categories, headlines)
        if category in ["politik", "wirtschaft", "finanzen"]
    ]
...
def _fail_invalid_inputs(starting_date, number_of_days):
    ...
    _fail_if_inputs_wrong_dtypes(
        starting_date=starting_date,
        number_of_days=number_of_days,
    )
    _fail_if_number_of_days_out_of_reach(
        starting_date=starting_date,
        number_of_days=number_of_days,
    )
...
def _fail_if_inputs_wrong_dtypes(starting_date, number_of_days):
    ...
    if (
        not isinstance(starting_date, datetime)
        or not isinstance(number_of_days, int)
        or isinstance(number_of_days, bool)
    ):
        msg = (
            "'starting_date' must be a datetime object and"
            " 'number_of_days' date must be an integer"
        )
        raise TypeError(
            msg,
        )
...
def _fail_if_number_of_days_out_of_reach(starting_date, number_of_days):
    ...
    if last_date_headlines > current_date:
        msg = "The number of days are not reachable. Last date lies in the future."
        raise ValueError(
            msg,
        )
...
def _fail_if_starting_date_is_before_2008(starting_date):
    ...
    if earliest_date > starting_date:
        msg = "'starting date' must lie after the year 2007."
