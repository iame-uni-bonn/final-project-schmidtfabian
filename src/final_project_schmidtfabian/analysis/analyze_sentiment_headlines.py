...
def analyze_sentiment_zero_shot_classification(dataframe_headlines):
    ...
    return _classify_sentiment_zero_shot(
        dataframe=dataframe_headlines,
        model=model,
        tokenizer=tokenizer,
...
def analyze_sentiment_finetuned_model(dataframe_headlines, location_finetuned_model):
    ...
    location_headlines = "headlines in a list"
    return _classify_sentiment_finetuned(
        dataframe=dataframe_headlines,
        model=model,
        tokenizer=tokenizer,
        column_to_analyze=location_headlines,
...
def _classify_sentiment_zero_shot(dataframe, model, tokenizer, column_to_analyze):
    ...
    dataframe_sentiment_analyzed = _create_sentiment_columns(
        dataframe=dataframe_sentiment_analyzed,
    )

    for index in dataframe_sentiment_analyzed.index:
        result = sentiment_classifier(
            dataframe_sentiment_analyzed.loc[index, column_to_analyze].tolist(),
        )
        dataframe_sentiment_analyzed = _add_results_to_dataframe(
            dataframe=dataframe_sentiment_analyzed,
            result=result,
            index=index,
        )

    dataframe_sentiment_analyzed[
        "sentiment_score_per_element"
    ] = dataframe_sentiment_analyzed["sentiment_score_per_element"].apply(_round_scores)
    dataframe_sentiment_analyzed["sentiment"] = dataframe_sentiment_analyzed[
        "sentiment"
...
def _fail_if_wrong_input(dataframe):
    ...
    _fail_if_elements_in_column_headlines_in_a_list_have_wrong_dtype(
        dataframe=dataframe,
    )
...
def _fail_does_not_contain_column_headlines_in_a_list(dataframe):
    ...
    if "headlines in a list" not in dataframe.columns:
        msg = "Column 'headlines in a list' not found in the dataframe."
...
def _fail_if_elements_in_column_headlines_in_a_list_have_wrong_dtype(dataframe):
    ...
    if not isinstance(headlines_list[0], str):
        msg = (
            "Column 'headlines in a list' is not a list containing strings,"
            f"its element's datatype is {column_dtype}."
        )
        raise ValueError(
            msg,
        )
...
def _fail_if_location_finetuned_model_is_not_a_filepath(location_finetuned_model):
